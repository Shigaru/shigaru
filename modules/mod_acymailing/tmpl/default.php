			...
			<?php }//endif dropdown
					...
					<?php foreach($fieldsToDisplay as $oneField){
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							if($displayOutside) echo '<td><label for="user_email_'.$formName.'">'.$emailCaption.'</label>'; ?>
							
								<input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" ';  if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $emailCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $emailCaption?>';"<?php } ?> class="inputbox search" type="text" name="user[email]" size="<?php echo $params->get('fieldsize',15)?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; elseif(!$displayOutside) echo $emailCaption; ?>" />
							<?php
						}elseif($oneField == 'html' AND empty($extraFields[$oneField])){
							echo JText::_('RECEIVE').JHTML::_('select.booleanlist', "user[html]" ,'',isset($identifiedUser->html) ? $identifiedUser->html : 1,JText::_('HTML'),JText::_('JOOMEXT_TEXT'),'user_html_'.$formName).'';
				...
				if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing::level(1)){ ?>
				 ...
				 if($params->get('showterms',false)){
					...
					<?php if(!$displayInline) echo '</tr><tr>';
						...
						<input class="button submit" type="submit" value="<?php echo $params->get('subscribetext',JText::_('SUBSCRIBECAPTION')); ?>" name="Submit" onclick="try{ return submitacymailingform('optin','<?php echo $formName;?>'); }catch(err){alert('The form could not be submitted, please make sure the jdoc:include type=head tag is in your template');return false;}"/>
						...
						<input class="button submit" type="button" value="<?php echo $params->get('unsubscribetext',JText::_('UNSUBSCRIBECAPTION')); ?>" name="Submit" onclick="return submitacymailingform('optout','<?php echo $formName;?>')"/>
						<?php } ?>
