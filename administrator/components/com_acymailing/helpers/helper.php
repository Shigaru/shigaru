...
class acymailing{
	...
	function absoluteURL($text){
		...
		$text = preg_replace('#(href|src|action|background)[ ]*=[ ]*\"(?!(https?://|\#|mailto:|/))(?:\.\./|\./)?#i','$1="'.ACYMAILING_LIVE,$text);
		$text = preg_replace('#(href|src|action|background)[ ]*=[ ]*\"(?!(https?://|\#|mailto:))/#i','$1="'.$mainurl,$text);
		return $text;
	}
	...
	function increasePerf(){
		if(acymailing::bytes(@ini_get('memory_limit')) < 60000000) @ini_set('memory_limit','64M');
