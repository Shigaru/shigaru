...
class FilterViewFilter extends JView
{
	...
	function form(){
		...
		$js .= "function submitbutton(pressbutton){
					...
					if(window.document.getElementById('filterinfo').style.display == 'none'){
		...
		if(acymailing::level(3)){
			$bar->appendButton( 'Popup', 'loadfilter', JText::_('ACY_LOAD'), acymailing::completeLink("filter&task=load",true),750,550);
			JToolBarHelper::spacer();
			JToolBarHelper::save();
			if(!empty($filter->filid)) $bar->appendButton( 'Link', 'new', JText::_('NEW'), acymailing::completeLink('filter&task=edit&filid=0') );
		}
		$bar->appendButton( 'Link', 'cancel', JText::_('CLOSE'), acymailing::completeLink('list') );
		JToolBarHelper::divider();
