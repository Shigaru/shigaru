	...
	function getLanguages() {
		...
		if (!isset($languages)) {
			...
			if (in_array($langs, $tables)){
                $v = AcesefUtility::getXmlText(JPATH_ADMINISTRATOR.'/components/com_joomfish/manifest.xml', 'version');

				// Get installed languages and add them to list
                if (version_compare($v, '2.2') == -1) {
				$langs = AceDatabase::loadObjectList("SELECT `id`, `shortcode`, `name` FROM `#__languages` WHERE `active` = '1' ORDER BY `ordering`");
                }
                else {
                    $langs = AceDatabase::loadObjectList("SELECT `lang_id`, `sef` AS shortcode, `title` AS name FROM `#__languages` WHERE `published` = '1' ORDER BY `lang_id`");
                }
