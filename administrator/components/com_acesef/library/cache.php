		...
		if (!isset($urls[$url])) {
            ...
            elseif ($ignore_multi_itemid) {
				...
				if (!empty($vars_itemid['option']) && !in_array($vars_itemid['option'], $skipped_components)) {
					...
					if(($vars_itemid["lang"] != 'es') || $vars_itemid["lang"] == 'en'){
						$row_3 = AceDatabase::loadObject("SELECT {$fields} FROM #__acesef_urls WHERE url_real = '{$url}%' AND url_real LIKE '%{$url_2}' AND params LIKE '%notfound=0%' LIMIT 1");
						}else{
							if($vars_itemid["lang"] == 'es'){
