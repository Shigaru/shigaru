...
class AcesefViewAcesef extends AcesefView {
	...
	function display($tpl = null) {
		...
		$this->toolbar->appendButton('Popup', 'help1', JText::_('ACESEF_COMMON_HELP'), 'http://www.joomace.net/support/docs/acesef/user-manual/control-panel?tmpl=component', 650, 500);
	...
	function showStatus($type) {
		...
		if (isset($status[$type])) {
			if ($type == "live_site") {
				...
				echo '&nbsp;';
				echo '<input type="button" onclick="sefStatus(\''.$type.'\', \'0\');" value="'.JText::_('ACESEF_COMMON_SAVE').'" />';
			}
			elseif ($type == "php") {
				if ($status[$type]) {
