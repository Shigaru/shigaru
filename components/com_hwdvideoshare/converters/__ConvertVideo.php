	...
	function convert($path_original, $path_new_flv, $filename_ext, $path_new_mp4, $gen_flv=1, $gen_mp4=1) {
		...
		if ($gen_flv == 1)
		{
			if ($c->encoder == "MENCODER")
			{
				...
				{
					$cmd_input_flv = "$s->mencoderpath $path_original -o $path_new_flv -of lavf -oac mp3lame -lameopts abr:br=$c->cnvt_abitrate -ovc lavc -lavcopts vcodec=flv:vbitrate=$c->cnvt_vbitrate:mbd=2:mv0:trell:v4mv:cbp:last_pred=3$men_keyframes -vf scale=$c->cnvt_fsize$wmvfix -srate $c->cnvt_asr $c->customencode";
				...
				if(!file_exists($path_new_flv) || (filesize($path_new_flv) == 0))
				{
					$cmd_input_flv = "$s->ffmpegpath -y -i $path_original -ab $c->cnvt_abitrate*1000 -ar $c->cnvt_asr -b $c->cnvt_vbitrate*1000 -s $c->cnvt_fsize -g $ffm_keyframes -keyint_min 25 $c->customencode $path_new_flv";
					@exec("$sharedlib $cmd_input_flv 2>&1", $cmd_output_flv);
				}
			}
		...
		if ($gen_mp4 == 1 && $c->uselibx264 == 1) {
			...
			$cmd_input_mp4 = "$s->ffmpegpath -y -chromaoffset 0 -i $path_original -an -pass 1 -s 640x480 -vcodec libx264 -b 768K -flags +loop -cmp +chroma -partitions 0 -me_method epzs -subq 1 -trellis 0 -refs 1 -coder 0 -me_range 16 -g $ffm_keyframes -keyint_min 25 -sc_threshold 40 -i_qfactor 0.71 -bt 768K -maxrate 1.5M -bufsize 10M -rc_eq 'blurCplx^(1-qComp)' -qcomp 0.6 -qmin 10 -qmax 51 -qdiff 4 -level 30 -aspect 1:1 $path_new_mp4";
			@exec("$sharedlib $cmd_input_mp4 2>&1", $cmd_output_mp4_p1);

			$cmd_input_mp4 = "$s->ffmpegpath -y -chromaoffset 0 -i $path_original -acodec libfaac -ab 128k -pass 2 -s 640x480 -vcodec libx264 -b 768K -flags +loop -cmp +chroma -partitions +parti4x4+partp8x8+partb8x8 -me_method umh -subq 5 -trellis 1 -refs 1 -coder 0 -me_range 16 -g $ffm_keyframes -keyint_min 25 -sc_threshold 40 -i_qfactor 0.71 -bt 768K -maxrate 1.5M -bufsize 10M -rc_eq 'blurCplx^(1-qComp)' -qcomp 0.6 -qmin 10 -qmax 51 -qdiff 4 -level 30 -aspect 16:9 $path_new_mp4";
