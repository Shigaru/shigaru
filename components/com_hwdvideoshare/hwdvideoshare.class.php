...
class hwd_vs_tools {
    ...
    function getAgoDate($date){
		...
		$cur_time = time();
		$diff = $cur_time - $date;
		$phrase = unserialize(_HWDVIDS_SHIGARU_AGOMETRIC);
		...
		if(substr($lang->getTag(),0,2) != 'en')
			$value = _HWDVIDS_SHIGARU_AGOTEXT.' '.$value;
			else 
				$value.= ' '._HWDVIDS_SHIGARU_AGOTEXT.' ';
		return $value;
	}
...
$app = & JFactory::getApplication();
   ...
   /**
    ...
    function getUserExtendedDetails($user_id){
		...
		$return = hwd_vs_tools::getSocialIconsMenu($cbUser,$showown);
		...
		if($cbUser->_cbuser->avatar && $cbUser->_cbuser->avatar != '')
			$smartyvs->assign("avatar", JURI::root()."images/comprofiler/".$cbUser->_cbuser->avatar);
			else
				$smartyvs->assign("avatar", JURI::root()."components/com_comprofiler/plugin/language/default_language/images/tnnophoto.jpg");
	...
	function getSocialIconsMenu($cbUser, $showown = 'no'){
		global $_CB_framework, $ueConfig;
		$oUserName = $cbUser->getField( 'username', null, 'csv', 'div','edit');
		$livesite = JURI::base();
		$myspacepage =  $cbUser->getField( 'cb_myspacepageURL', null, 'csv', 'div','edit');
		$ocode ='';
		$return ='';
		$haveany = false;
		if($myspacepage  !=''){
		...
		$twitterpage = $cbUser->getField( 'cb_twitterpageURL', null, 'csv', 'div','edit');
		if($twitterpage  !=''){
		...
		$facebook = $cbUser->getField( 'cb_facebookURL', null, 'csv', 'div','edit');
		if($facebook  !=''){
		...
		$linkedin = $cbUser->getField( 'cb_linkedinURL', null, 'csv', 'div','edit');
		if($linkedin  !=''){
		...
		$google = $cbUser->getField( 'cb_googleURL', null, 'csv', 'div','edit');
		if($google  !=''){
   ...
   function getUserVideoCount($user_id){
    ...
    function getMyWatchHistory($guid= 'no'){
		...
		if ($sort_by == 'category')
				...
				{
					$orderVideos = " ORDER BY history.date DESC";
				}
		
		$query = 'SELECT history.date, video.*'
               	. ' FROM #__hwdvidslogs_views AS history, #__hwdvidsvideos as video '
				. $where
				. $orderVideos
				;
    ...
    function getMyLikedVideos(){
		...
		if ($sort_by == 'category')
				...
				{
					$orderVideos = " ORDER BY likes.date_liked DESC";
				}
		
		$query = 'SELECT likes.date_liked, video.*'
               	. ' FROM #__hwdvidslikes AS likes, #__hwdvidsvideos as video '
				. $where
				. $orderVideos
				;
    ...
    function getMyLearnLaterVideos(){
		global $smartyvs,$mainframe, $Itemid, $hwdvs_joinv, $hwdvs_selectv;
		$c = hwd_vs_Config::get_instance();
		$db = & JFactory::getDBO();
		$otheruser = Jrequest::getVar( 'guid', 'no' );
		...
		if ($sort_by == 'category')
				...
				{
					$orderVideos = " ORDER BY learnlater.date_added DESC";
				}
		
		$query = 'SELECT learnlater.date_added, video.*'
               	. ' FROM #__hwdvidsplaylists_videos AS learnlater, #__hwdvidsvideos as video '
				. $where
				. $orderVideos
				;
	...
	function getAboutMeData(){
		...
		$cb_otherlanguages = explode(",", $cbUser->getField( 'cb_otherlanguages' , null, 'csv', 'profile' ));
		$finalcb_otherlanguages = hwd_vs_tools::constantizeArray($cb_otherlanguages);				
		$cb_favmusicgenre = explode(",", $cbUser->getField( 'cb_favmusicgenre' , null, 'csv', 'profile' ));
		$finalcb_favmusicgenre = hwd_vs_tools::constantizeArray($cb_favmusicgenre);				
		$cb_contriestravelled = explode(",", $cbUser->getField( 'cb_contriestravelled' , null, 'csv', 'profile' ));
		...
		$smartyvs->assign("cb_nativelang", constant($cbUser->_cbuser->cb_nativelang));
		$smartyvs->assign("languages", $finalLang);
		$smartyvs->assign("cb_otherlanguages", $finalcb_otherlanguages);
	...
	function generateSuggestion(){
		return '<div id="noresultswrapper" class="fontbold pad12 w100 tcenter f150"><i class="icon-frown icon-spin"></i> '._HWDVIDS_INFO_NMV.'</div>';
		}
