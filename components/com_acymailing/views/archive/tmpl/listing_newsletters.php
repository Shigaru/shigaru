		...
		<tbody>
			...
			?>
				<tr class="<?php echo "sectiontableentry$k".$this->values->suffix; ?>">
					...
					<td>
						<a <?php if($this->config->get('open_popup')){ echo 'class="modal" rel="{handler: \'iframe\', size: {x: 750, y: 550}}"'; } ?> href="<?php echo acymailing::completeLink('archive&task=view&listid='.$this->list->listid.'-'.$this->list->alias.'&mailid='.$row->mailid.'-'.$row->alias,(bool)$this->config->get('open_popup')); ?>">
						...
						<?php if($this->access->frontEndManament){
							...
							<span class="acyeditbutton"><a href="<?php echo acymailing::completeLink('newsletter&task=edit&mailid='.$row->mailid.'&listid='.$this->list->listid); ?>"><img src="<?php if($row->visible AND $row->published) echo JURI::base(true)."/images/M_images/edit.png"; else echo JURI::base(true)."/images/M_images/edit_unpublished.png"; ?>"/></a></span>
