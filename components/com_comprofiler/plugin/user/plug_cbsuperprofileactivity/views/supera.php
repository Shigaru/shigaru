...
//$subdircheck = $this->url_exists($cb);
// *********************************************  
...
if ($datetimepattern) {
  ...
  $ago = $this->Ago($fromtime,$nowexact);
...
// *********************************************
  if ($specific &&!$friendactivity && $userid!=$user->id) {$startshow=0;}  
// *********************************************  
...
$startoutput .= "<div class=\"activity_inner\">";
if ($datetimepattern) {
  $day=$ago[grday];
  $time=$ago[grtime];
} else { $day = $this->newGetEventDate($day); }

if ($showdaily && $day!=$tempday) {
  $startoutput .= "<span class=\"activity_day\">".$day."</span>";
  if ($infotext) $startoutput .= "<br/>";
}
// *********************************************  
...
// *********************************************  
if ($infotext) { 
  $startoutput .= "<span class=\"activity_infotext\">".$function;
// *********************************************  
if ($infotext==1) { // for timestamp
  if (!$showdaily) {
    $startoutput .= " - ".$day.$time."</span>"; 
  } else {
    $startoutput .= $time."</span>"; 
  }
} else {$startoutput .= "</span>";}
// *********************************************  
}
// *********************************************  
...
include $grpath.'/views/defaultdisplay.php';
$endoutput .= "</div>";
if ($spacerline) $endoutput .= "<hr/>";

//if ($friendactivity) $output.= "Actidate:$actidate, User:$userid, Flag:$flag<br/>"; 

$return .= $startoutput.$output.$endoutput;
...
if ($noactiv==1 && $modulename!=5) {
  if ($temp_increase==0) {
    if ($friendactivity) {
      $return = _ACT_NO_FRIEND_TAB;
    } else {
      $return = _ACT_NO_ACTIVITY_TAB;
    }
  }
